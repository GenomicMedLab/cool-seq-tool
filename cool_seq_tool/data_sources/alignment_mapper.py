"""Module containing alignment methods for translating to and from different reference
sequences.
"""
...
class AlignmentMapper:
    ...
    async def p_to_c(
        ...
        :return: Tuple containing cDNA representation (accession, codon range
            positions for corresponding change, cds start site) if able to translate.
            Will return positions as inter-residue coordinates. If unable to translate,
            returns `None`. Otherwise, if unable to translate to cDNA representation,
            returns warning description. Else ``None``.
        """
        # Get cDNA accession
    ...
    async def _get_cds_start(self, c_ac: str) -> Tuple[Optional[int], Optional[str]]:
        ...
        :return: Tuple containing CDS start site if found, else `None`, in first slot.
            The second slot contains a warning description if unable to get CDS start,
            otherwise ``None``.
        """
    ...
    async def c_to_g(
        ...
        :param cds_start: Coding start site. If not provided, this will be
            computed.
        :param residue_mode: residue mode for ``c_start_pos`` and ``c_end_pos``
        :param target_genome_assembly: Genome assembly to get genomic data for
        :return: Tuple containing genomic representation (ac, positions) if able to
            translate (will return positions as inter-residue coordinates), and None.
            If unable to translate to genomic representation, returns None and warning.
        """
        if any((
    ...
    async def p_to_g(
        ...
        :param residue_mode: Residue mode for `p_start_pos` and `p_end_pos`.
        :param target_genome_assembly: Genome assembly to get genomic data for
        :return: Tuple containing in first slot genomic representation (ac, positions)
            if able to translate (will return positions as inter-residue coordinates),
            else ``None``, as well as any warnings in second slot. The genomic data will
            always return inter-residue coordinates.
        """
