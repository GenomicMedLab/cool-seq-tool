...
async def test_db():
    ...
    class TestUTADatabase:
        ...
        async def get_genomic_coords(self, tx_ac: str, start_exon: int,
                                     ...
                                     end_exon_offset: int = 0,
                                     gene: str = None):
            return await self.test_db.get_genomic_coords(
...
async def test_uta_source(test_db, tpm3_exon1_exon8, ntrk1_exon10_exon17):
    ...
    # TPM3
    resp = await test_db.get_genomic_coords('NM_152263.3', 0, 8)
    assert resp == tpm3_exon1_exon8

    resp = await test_db.get_genomic_coords('NM_152263.3       ', 0, 8)
    assert resp == tpm3_exon1_exon8

    resp = await test_db.get_genomic_coords('NM_152263.3', 0, 8, gene="TPM3")
    assert resp == tpm3_exon1_exon8
    ...
    resp = await test_db.get_genomic_coords('NM_152263.3', 0, 0, gene="tpm3")
    ...
    resp = await test_db.get_genomic_coords('NM_152263.3', 3, 8,
                                            start_exon_offset=3,
                                            end_exon_offset=5)
    ...
    # NTRK1
    resp = await test_db.get_genomic_coords('NM_002529.3', 10, 0)
    assert resp == ntrk1_exon10_exon17

    resp = await test_db.get_genomic_coords('NM_002529.3', 10, 0, gene="NTRK1")
    assert resp == ntrk1_exon10_exon17
...
async def test_no_matches(test_db):
    ...
    resp = await test_db.get_genomic_coords('NM_152263.3', 0, 22)
    assert resp is None

    # Start > End
    resp = await test_db.get_genomic_coords('NM_152263.3', 8, 1)
    assert resp is None

    # End < Start
    resp = await test_db.get_genomic_coords('NM_152263.3', 7, 6)
    assert resp is None

    # Transcript DNE
    resp = await test_db.get_genomic_coords('NM_12345.6', 7, 0)
    assert resp is None

    # Index error for invalid exon
    resp = await test_db.get_genomic_coords('NM_12345.6', -1, 0)
    assert resp is None

    # Gene that does not match transcript
    resp = await test_db.get_genomic_coords('NM_152263.3', 8, 1, gene='NTKR1')
    assert resp is None

    # No transcript given
    resp = await test_db.get_genomic_coords(None, 8, 1, gene='NTKR1')
    assert resp is None

    resp = await test_db.get_genomic_coords('', 8, 1, gene='NTKR1')
    assert resp is None
