...
def tpm3_exon1_g(tpm3_exon1):
    ...
    params = {
        ...
        "genomic_accession": tpm3_exon1.genomic_accession,
...
def tpm3_exon8_g(tpm3_exon8):
    ...
    params = {
        ...
        "genomic_accession": tpm3_exon8.genomic_accession,
...
def genomic_tx_seg_service_checks(actual, expected=None, is_valid=True):
    ...
    if is_valid:
        ...
        assert actual.genomic_accession == expected.genomic_accession
        ...
        assert actual.genomic_accession is None
...
async def test_get_get_exons_coords(test_egc_mapper, nm_152263_exons_genomic_coords):
    """Test that _get_exons_coords works correctly."""
    resp = await test_egc_mapper._get_exons_coords("NM_152263.4", "NC_000001.11")
    assert resp == nm_152263_exons_genomic_coords

    # Invalid transcript accession given chromosome accession
    resp = await test_egc_mapper._get_exons_coords("NM_001105539.3", "NC_000001.11")
    assert resp == []
...
async def test_braf(test_egc_mapper, mane_braf):
    ...
    """
    inputs = {
        "genomic_accession": "NC_000007.13",
...
async def test_transcript_to_genomic(
    ...
    genomic_tx_seg_service_checks(resp, tpm3_exon8_g)
...
async def test_valid_inputs(test_egc_mapper):
    ...
    resp = await test_egc_mapper.genomic_to_tx_segment(**inputs)
    assert all((resp.gene, resp.genomic_accession, resp.tx_ac, resp.seg_start))
    ...
    resp = await test_egc_mapper.genomic_to_tx_segment(**inputs)
    assert all((resp.gene, resp.genomic_accession, resp.tx_ac, resp.seg_end))
    ...
    resp = await test_egc_mapper.genomic_to_tx_segment(**inputs)
    assert all(
        (resp.gene, resp.genomic_accession, resp.tx_ac, resp.seg_start, resp.seg_end)
    )
...
async def test_invalid(test_egc_mapper):
    ...
    resp = await test_egc_mapper.genomic_to_tx_segment(
        genomic_start=154191901, genomic_accession="NC_000001.11"
    )
    genomic_tx_seg_service_checks(resp, is_valid=False)
    ...
    genomic_tx_seg_service_checks(resp, is_valid=False)
    assert resp.errors == ["Unable to get exons for NM_12345.6"]
